...
contract EmbeddedParliaBlockVerifier is ParliaBlockVerifier {
    ...
    function _extractParliaSigningData(bytes calldata blockProof, uint256 chainId, uint32 epochLength) internal view override returns (VerifiedParliaBlockResult memory) {
        bytes memory input = abi.encode(chainId, blockProof, epochLength);
